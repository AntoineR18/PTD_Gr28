...
df_2016 = df_athletes_merged[
    (df_athletes_merged["Year"] == 2016) & (df_athletes_merged["Medal"].notna())
].copy()

# Détection des épreuves collectives
# (plusieurs équipes différentes dans une même épreuve)
collective_events_2016 = (
    df_2016.groupby("Event").filter(lambda x: x["Team"].nunique() > 1)["Event"].unique()
)
...
df_2016_dedup = pd.concat(
    [
        ...
        df_2016[df_2016["IsTeamEvent"]].drop_duplicates(
...
medailles_par_pays_2016 = (
    df_2016_dedup.groupby("region")["Medal"].value_counts().unstack(fill_value=0)
)
...
print(
    f"La borne inférieur est : {borne_inferieure}\n"

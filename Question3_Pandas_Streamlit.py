...
season = st.radio("Saison :", ["Summer", "Winter"],
...
selected_discipline = st.selectbox("Filtrer par discipline (facultatif) :",
                                   ["Toutes"] + all_disciplines)
...
medal_counts = (unique_medals.groupby('Medal').size()
                .reindex(['Gold', 'Silver', 'Bronze'], fill_value=0))
medal_counts.index = medal_counts.index.map({"Gold": "Or", "Silver": "Argent",
                                             "Bronze": "Bronze"})
medal_counts['Total'] = medal_counts.sum()

st.subheader("\U0001F4CA Tableau des médailles")
st.dataframe(medal_counts.reset_index()
             .rename(columns={0: 'Nombre', 'index': 'Médaille'}))
...
for sport in sorted(unique_medals['Sport'].unique()):
    with st.expander(sport):
        details = unique_medals[unique_medals['Sport'] == sport][['Event',
                                                                  'Medal',
                                                                  'Name']]
        ...
        details['Médaille'] = details['Médaille'].map({"Gold": "Or",
                                                       "Silver": "Argent",
                                                       "Bronze": "Bronze"})
